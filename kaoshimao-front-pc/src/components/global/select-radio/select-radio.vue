    ...
    <el-card class="box-card" v-if="content.qtype">
      ...
      <div class="clearfix">
        <div class="select-left pull-left" v-if="content">
          <!-- v-model="mode===3?hasAnswer:selfAnswer" -->
          <el-radio-group v-if="content.qtype == 1 || content.qtype == 3" v-model="isRecite?hasAnswer:selfAnswer" @change="apply">
            <el-radio 
            v-for="item in JSON.parse(content.options)"
            :key="item.Value"
            :label="item.Key"
            :disabled="isRecite">
            <!-- <img v-on:click.prevent="previewImg('http://www.ruanyifeng.com/blogimg/asset/2016/bg2016041202.jpg')" class="select-img" src="http://www.ruanyifeng.com/blogimg/asset/2016/bg2016041202.jpg" alt=""> -->
            <!-- {{item.Value}} -->
            <span v-html="item.Value"></span>
            </el-radio>
          </el-radio-group>
          <el-checkbox-group v-if="content.qtype == 2" v-model="isRecite?checkBoxAnswerList:checkBoxAnswer">
            <el-checkbox
            v-for="item in JSON.parse(content.options)"
            :key="item.Value"
            :label="item.Key">
            ...
            <el-button style="margin-bottom:20px" @click="apply">提交答案</el-button>
          </el-checkbox-group>
          <div v-if="showEditor">
            ...
            <quill-editor style="margin-bottom: 20px;"
      ...
      <div v-if="!isSimulation" class="practice-ctrl">
        <div v-if="isRecite || showAns">
          <div class="results">
            ...
            <span class="results-text err">{{ content.answer }}</span>
          </div>
          <hr>
        ...
        <div v-if="!isSimulation && !result && !search">
          <hr>
          <el-button type="primary" size="medium" :disabled="topicNum==0" @click="goTopic(1)">上一题</el-button>
          <el-button type="primary" @click="goTopic(2)" v-if="pages" :disabled="topicNum==pages.total-1" size="medium">下一题</el-button>
          ...
          <span class="page-jump gray" v-if="pages.total">跳转至&nbsp;
            <el-input class="page-inp gray" type="number" v-model="jumNum" @change="goTopic(3,jumNum)"></el-input>
          &nbsp;/ {{pages.total}}&nbsp;题</span>
          <el-button class="pull-right" @click="applayError" icon="el-icon-warning">纠错</el-button>
          <el-button class="pull-right" style="margin-right:10px" icon="el-icon-star-off">
