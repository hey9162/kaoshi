      ...
      <div v-if="init">  
        ...
        <div class="pull-left sheet">
          <el-card>
            ...
            <hr>
            <el-button type="primary" size="small" @click="submit" class="full-btn">交卷</el-button>
          </el-card>
          <el-card class="tip-time">
            <div class="clearfix">
              <span class="gray pull-left">已用时间： {{time.timeStr}}</span>
              <el-button class="pull-right" size="small" @click="timeCtrl(0)" v-if="!isStop">暂停</el-button>
              <el-button class="pull-right" size="small" @click="timeCtrl(1)" v-if="isStop">继续</el-button>
            </div>
          </el-card>
        </div>
      </div>
  ...
  methods: {
    ...
    goTopic(data,index){
      switch (data.type) {
        ...
        case 2:
          /**修改模拟考试下一题跳转404页面 */
          if(this.contents['rows'][this.topicNum+1]){
            this.topicNum++;
          }else{
            this.submit()
          }
    ...
    submitConfirm(){
      ...
      setSession('simulationInfo',data )
      this.$router.push({ path: '/online/simulation/result'})
    },
    initData(){
      if(this.data){
